...
export const PuzzleRush: React.FC = () => {
  ...
  // Initialize sound service and preload audio packs
  useEffect(() => {
    soundService.initialize();
  }, []);

  // Low-time alarm: play a ticking sound when timeLeft is between 1 and 10 seconds
  useEffect(() => {
    if (timeLeft <= 10 && timeLeft > 0) {
      soundService.play('low_time_alarm');
    }
  }, [timeLeft]);
