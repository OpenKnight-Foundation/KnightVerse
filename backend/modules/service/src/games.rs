...
mod tests {
    ...
    #[tokio::test]
    async fn list_games_issued_against_supplied_connection() {
        let mock_game = make_mock_game();

        // Build a mock database that returns a count of 0 then one game row
        let mock_db = MockDatabase::new(DbBackend::Postgres)
            // count query
        ...
        .await
        .expect("list_games_on should succeed");
    ...
    async fn create_game_issues_insert() {
        ...
        let mock_db = MockDatabase::new(DbBackend::Postgres)
            .append_query_results(vec![vec![mock_game_clone]])
            .into_connection();
