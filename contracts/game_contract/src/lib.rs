...
const DISPUTE_VOTES: Symbol = symbol_short!("D_VOTE"); // Map<u64, Map<Address, DisputeVote>>
const DISPUTE_MULTISIG: Symbol = symbol_short!("D_MS"); // Map<u64, MultiSigDispute>

// Dynamic fee split (#978)
const FEE_SPLIT_CONFIG: Symbol = symbol_short!("FS_CFG"); // FeeSplitConfig
const FEE_SPLITS: Symbol = symbol_short!("FSPLIT"); // Map<u64, FeeSplit> (per-game splits)

// Spectator tipping (#979)
const SPECTATOR_TIPS: Symbol = symbol_short!("SP_TIP"); // Map<u64, Vec<SpectatorTip>>
const TIP_RECIPIENTS: Symbol = symbol_short!("TP_REC"); // Map<Address, i128>

// Anti-cheat security deposit (#980)
const CHEAT_DEPOSITS: Symbol = symbol_short!("CH_DEP"); // Map<Address, CheatDeposit>
const DEPOSIT_CONFIG: Symbol = symbol_short!("DEP_CFG"); // DepositConfig
...
// ────────────────────────────────────────────────────────────────────────────
// Dynamic fee split types (#978)
// ────────────────────────────────────────────────────────────────────────────

#[contracttype]
#[derive(Clone, Debug)]
pub struct FeeSplitConfig {
    pub platform_fee_bips: u32,    // Platform's base fee in basis points
    pub creator_share_bips: u32,   // Game creator's share
    pub winner_share_bips: u32,    // Winner's share
    pub treasury_share_bips: u32,  // Treasury's share
}
...
// ────────────────────────────────────────────────────────────────────────────
// Spectator tipping types (#979)
// ────────────────────────────────────────────────────────────────────────────

#[contracttype]
#[derive(Clone, Debug)]
pub struct SpectatorTip {
    pub tip_id: u64,
    pub game_id: u64,
    pub spectator: Address,
    pub recipient: Address, // Player being tipped
    pub amount: i128,
    pub timestamp: u64,
...
// ────────────────────────────────────────────────────────────────────────────
// Anti-cheat security deposit types (#980)
// ────────────────────────────────────────────────────────────────────────────

#[contracttype]
#[derive(Clone, Debug)]
pub struct CheatDeposit {
    pub player: Address,
    pub deposit_amount: i128,
    pub deposited_at: u64,
    pub is_seized: bool,
    pub seizure_reason: Option<Bytes>,
}
...
pub struct DepositConfig {
    ...
    pub seizure_threshold: u32, // Number of cheat reports to trigger seizure
    pub refund_period: u64,     // Ledger sequences before deposit can be refunded
}
...
pub enum ContractError {
    ...
    ContractPaused = 39,
    /// Dispute not found (#977)
    DisputeNotFound = 40,
    /// Already voted on this dispute (#977)
    AlreadyVoted = 41,
    /// Dispute threshold not met (#977)
    ThresholdNotMet = 42,
    /// Invalid fee split configuration (#978)
    InvalidFeeSplit = 43,
    /// Spectator tipping not enabled (#979)
    TippingNotEnabled = 44,
    /// Cannot tip yourself (#979)
    CannotTipSelf = 45,
    /// Deposit below minimum (#980)
    DepositTooLow = 46,
    /// Deposit above maximum (#980)
    DepositTooHigh = 47,
    /// No deposit found (#980)
    NoDeposit = 48,
    /// Deposit still locked (#980)
    DepositLocked = 49,
}
