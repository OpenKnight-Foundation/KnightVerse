...
const PUZZLE_BOUNTIES: Symbol = symbol_short!("PZL_BNT"); // Map<u64, PuzzleBounty>
const BOUNTY_COUNTER: Symbol = symbol_short!("BNT_CNT"); // u64

// Time-decay penalties (#986)
const PENALTIES: Symbol = symbol_short!("PENALTY"); // Map<Address, PenaltyRecord>
const DECAY_CONFIG: Symbol = symbol_short!("DCAY_CFG"); // DecayConfig

// Token-gated VIP (#987)
const VIP_HOLDERS: Symbol = symbol_short!("VIP_HLD"); // Map<Address, VipTier>
const VIP_CONFIG: Symbol = symbol_short!("VIP_CFG"); // VipConfig

// Soulbound trophies (#994)
const TROPHIES: Symbol = symbol_short!("TROPHY"); // Map<Address, Vec<Trophy>>
const TROPHY_REGISTRY: Symbol = symbol_short!("TR_REG"); // Map<u64, TrophyMetadata>
...
pub enum ContractError {
    ...
    ContractPaused = 39,
    /// Puzzle bounty not found (#982)
    BountyNotFound = 40,
    /// Puzzle bounty already claimed (#982)
    BountyAlreadyClaimed = 41,
    /// Player already has this trophy (#994)
    TrophyAlreadyOwned = 42,
    /// Not the trophy owner (#994)
    NotTrophyOwner = 43,
    /// VIP tier not found (#987)
    VipTierNotFound = 44,
    /// Insufficient VIP tokens (#987)
    InsufficientVipTokens = 45,
    /// Player not banned, cannot reinstate (#986)
    NotBanned = 46,
}
